...
A fresh empty Docker volume is populated from the image-baked model store, so `qwen3.5:9b` is available without a first-start model download.
...
The baked `qwen3.5:9b` model is the default general-purpose text model. The CLI never silently switches models or downloads a vision model. When image input is requested, the selected model must report Ollama's `vision` capability.
...
curl http://127.0.0.1:11434/api/generate \
  ...
  -d '{"model":"qwen3.5:9b","prompt":"Reply with OK only.","stream":false}'
```
